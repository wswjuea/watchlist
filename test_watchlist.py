...
class WatchlistTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        db.session.remove()  # 清除数据库回话session
        """
        在实际web应用中,session是scoped类型的session,每个request handler会自主创建并销毁自己的session,每个request会创建一个新的session;但是在单元测试的环境中,没有完全复现真实的请求情景,在request的最后没有执行db.session.remove()操作,所以要写入tearDown中
        """
    ...
    def test_404_page(self):
        response = self.client.get('/nothing')  # 未设置视图函数的路径
